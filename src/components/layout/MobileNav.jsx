...
export default function MobileNav() {
  const { currentView, setView, toggleCartDrawer } = useStore()
  ...
  return (
    <div className="sm:hidden fixed bottom-0 left-0 right-0 z-45 bg-white border-t border-gray-100 shadow-[0_-2px_10px_rgba(0,0,0,.06)]" role="navigation" aria-label="Mobile navigation" style={{ paddingBottom: 'env(safe-area-inset-bottom, 4px)', paddingTop: '6px' }}>
      <div className="flex justify-around items-center">
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => handleClick(tab)} className={`flex flex-col items-center gap-0.5 px-2 py-1 text-[.7rem] font-semibold transition relative ${isActive(tab.id) ? 'text-brand' : 'text-secondary'}`}>
            {tab.isCart ? (
