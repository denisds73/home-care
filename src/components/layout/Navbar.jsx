    ...
    <nav className="sticky top-0 z-50 bg-white transition-shadow" role="navigation" aria-label="Main navigation" style={{ boxShadow: scrolled ? '0 4px 16px rgba(0,0,0,.1)' : '0 2px 4px rgba(0,0,0,.06)' }}>
      ...
      <div className="border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2.5 flex items-center gap-3 sm:gap-4">
          <button onClick={() => setView('home')} className="flex items-center gap-2 shrink-0" aria-label="HomeCare home">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {cartCount > 0 && (
              <button onClick={toggleCartDrawer} className="relative p-2 rounded-lg hover:bg-gray-100 transition" aria-label="Shopping cart">
            ...
            <button onClick={() => setView('home')} className="hidden sm:inline-flex btn-base btn-secondary px-3 py-1.5 text-xs">Customer</button>
            <button onClick={() => useStore.getState().adminUnlocked ? setView('admin') : useStore.setState({ adminAuthOpen: true })} className="hidden sm:inline-flex btn-base btn-secondary px-3 py-1.5 text-xs">Admin</button>
          </div>
        </div>
      </div>
      ...
      <div className="hidden sm:block border-b border-gray-100 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
        <div className="max-w-7xl mx-auto px-3 sm:px-4 flex">
          <button onClick={() => setView('home')} className={`flex items-center gap-1.5 px-4 py-2.5 text-[.8rem] font-semibold whitespace-nowrap border-b-[2.5px] transition ${currentView === 'home' ? 'text-brand border-brand' : 'text-secondary border-transparent hover:text-primary hover:bg-gray-50'}`}>
