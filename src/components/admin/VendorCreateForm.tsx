...
import type { CreateVendorPayload, Vendor } from '../../types/domain'

const PHONE_RE = /^[6-9]\d{9}$/
const PIN_RE = /^\d{6}$/
const GST_RE = /^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$/
export const DEMO_VENDOR_PASSWORD = 'demo123'
...
type VendorCreateFormProps = {
  variant: 'page' | 'dialog'
  /** Called after successful create when `variant` is `dialog` */
...
export function VendorCreateForm({
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof FormState | 'pin_codes' | 'category_ids', string>>
  >({})

  const idPrefix = variant === 'dialog' ? 'vc_d' : 'vc_p'
    ...
    variant === 'page' ? (
      <>
        ...
        <p className="text-xs text-secondary mt-2">
          Demo login provisioning: vendor can sign in with onboarding email and
          password{' '}
      ...
      <p className="text-xs text-secondary -mt-1 mb-1">
        Demo login: onboarding email and password{' '}
        <span className="font-semibold text-primary">{DEMO_VENDOR_PASSWORD}</span>
        .
      </p>
    )
  ...
  const outer = variant === 'page' ? 'fade-in mx-auto w-full max-w-3xl space-y-6' : 'space-y-5'

  return (
    <div className={outer}>
      {variant === 'page' ? <div>{intro}</div> : intro}
      ...
      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        <section className="glass-card p-5 space-y-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="label-base" htmlFor={`${idPrefix}_company_name`}>
              Company Name
            </label>
            <input
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label
                ...
                htmlFor={`${idPrefix}_contact_number`}
              >
                Contact Number
              </label>
              <input
                id={`${idPrefix}_contact_number`}
            ...
            <div className="flex flex-col gap-1">
              <label className="label-base" htmlFor={`${idPrefix}_email`}>
                Email
              </label>
              <input
                id={`${idPrefix}_email`}
        ...
        <section className="glass-card p-5 space-y-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="label-base" htmlFor={`${idPrefix}_city`}>
              City
            </label>
            <input
              id={`${idPrefix}_city`}
          ...
          <div className="flex flex-col gap-2">
            <label className="label-base" htmlFor={`${idPrefix}_pin_draft`}>
              PIN Codes
            </label>
            <div className="flex gap-2">
              <input
                id={`${idPrefix}_pin_draft`}
        ...
        <section className="glass-card p-5 space-y-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="label-base" htmlFor={`${idPrefix}_gst_number`}>
              GST Number
            </label>
            <input
              id={`${idPrefix}_gst_number`}
