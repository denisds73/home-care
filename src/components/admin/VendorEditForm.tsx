...
export function VendorEditForm({
  ...
  const idPrefix = variant === 'dialog' ? 've_d' : 've_p'
  ...
  const load = useCallback(async () => {
    try {
      setIsLoading(true)
      setError(null)
      const v = await vendorService.get(vendorId)
      hydrate(v)
    } catch (err) {
  ...
  const applyStatus = async (status: VendorStatus) => {
    try {
      const updated = await vendorService.updateStatus(vendorId, status)
      hydrate(updated)
  ...
  if (isLoading) {
    return (
      <div
        className={
          variant === 'page'
            ? 'fade-in mx-auto w-full max-w-3xl space-y-4'
            : 'py-2'
        }
      >
  ...
  if (error || !vendor) {
    const body = (
      <div
        className={
          variant === 'page'
            ? 'glass-card mx-auto w-full max-w-lg p-8 text-center'
            : 'p-4 text-center'
        }
      >
  ...
  const outer =
    variant === 'page'
      ? 'fade-in mx-auto w-full max-w-3xl space-y-6'
      : 'space-y-5'
      ...
      <form onSubmit={handleSave} className="space-y-5" noValidate>
        <section className="glass-card p-5 space-y-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="label-base" htmlFor={`${idPrefix}_company`}>
              Company Name
            </label>
            <input
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label className="label-base" htmlFor={`${idPrefix}_phone`}>
                Contact Number
              </label>
              <input
                id={`${idPrefix}_phone`}
        ...
        <section className="glass-card p-5 space-y-4">
          ...
          <div className="flex flex-col gap-2">
            <label className="label-base" htmlFor={`${idPrefix}_pin`}>
              PIN Codes
            </label>
        ...
        <section className="glass-card p-5 space-y-4">
          ...
          <div className="flex flex-col gap-1">
            <label className="label-base" htmlFor={`${idPrefix}_gst`}>
              GST Number
            </label>
            <input
              id={`${idPrefix}_gst`}
