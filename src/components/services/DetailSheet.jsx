...
export default function DetailSheet() {
  const { detailSheetOpen, detailServiceId, closeDetailSheet, services, addToCart, removeFromCart, showToast, toggleCartDrawer } = useStore()
  const qty = useStore(s => s.cart.find(c => c.service.id === s.detailServiceId)?.qty || 0)
  ...
  return (
    <>
      <div className="fixed inset-0 z-[54] bg-black/40 transition-opacity" onClick={closeDetailSheet} />
      <div className="fixed bottom-0 left-0 right-0 z-[56] bg-white rounded-t-2xl shadow-[0_-8px_40px_rgba(0,0,0,.15)] max-h-[85vh] overflow-y-auto transition-transform" style={{ animation: 'slideUp .35s ease-out' }}>
        ...
        <button onClick={closeDetailSheet} className="absolute top-3 right-4 w-8 h-8 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition z-10" aria-label="Close details">
        ...
        <div className="px-5 pb-6">
          ...
          <div className="sticky bottom-0 bg-white pt-3 pb-2 border-t -mx-5 px-5">
            {qty === 0 ? (
              <button onClick={handleAdd} className="bg-brand text-white w-full py-3.5 rounded-xl font-bold text-sm hover:bg-brand-dark transition">
              ...
              <div className="flex items-center justify-between">
                <div className="inline-flex items-center border-2 border-brand rounded-lg overflow-hidden scale-110">
                  <button onClick={() => removeFromCart(svc.id)} className="w-8 h-8 bg-brand text-white flex items-center justify-center font-bold">−</button>
                  <span className="w-9 text-center font-bold text-sm text-brand-dark">{qty}</span>
                  <button onClick={handleAdd} className="w-8 h-8 bg-brand text-white flex items-center justify-center font-bold">+</button>
                </div>
                <button onClick={() => { closeDetailSheet(); toggleCartDrawer() }} className="bg-brand text-white px-6 py-3 rounded-xl font-bold text-sm hover:bg-brand-dark transition">View Cart</button>
              </div>
            )}
          </div>
        </div>
