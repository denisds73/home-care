      ...
      >
        ...
        <div className="hidden md:flex flex-col gap-4 mt-8 max-w-xs">
          {[
            ...
            { text: 'Vendor management and approval workflows', icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z' },
          ].map((item, i) => (
        ...
        <div className="glass-card no-hover p-7 md:shadow-none md:border-0 md:bg-transparent">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3" noValidate>
            ...
            <div className="flex flex-col gap-1">
              <label htmlFor="admin-email" className="text-xs font-semibold text-secondary">Email address</label>
              ...
              {emailError && (
                <p id="admin-email-error" className="text-xs text-error fade-in">{emailError}</p>
              )}
            </div>

            {/* Password */}
            <div className="flex flex-col gap-1">
              <label htmlFor="admin-password" className="text-xs font-semibold text-secondary">Password</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted" aria-hidden="true">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              {passwordError && (
                <p id="admin-password-error" className="text-xs text-error fade-in">{passwordError}</p>
              )}
            </div>
