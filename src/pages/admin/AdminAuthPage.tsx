...
/**
 * Standalone admin auth page (for direct URL access to /admin/auth).
 * Also used by the ProtectedRoute guard.
 */
export default function AdminAuthPage() {
  ...
  return (
    <div className="min-h-screen bg-surface flex items-center justify-center p-4 fade-in">
      <Modal isOpen={true} onClose={() => navigate('/')} maxWidth="max-w-xs">
        <div className="p-6 text-center">
          <svg className="w-10 h-10 mx-auto mb-3 text-brand" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
          ...
          <button onClick={verify} className="btn-base btn-primary text-white w-full py-3 rounded-xl font-semibold text-sm">
            Unlock Admin
          </button>
          <button onClick={() => navigate('/')} className="btn-base btn-secondary w-full py-2 text-xs mt-2">
            Cancel
          </button>
        </div>
      </Modal>
